...
class Character {
        ...
        function step(timestamp) {
            ...
            if(timestamp - startTime >= 100){
                if (state == 'left') {
                    ...
                    if (thiss.y > 0 && thiss.state != "wall_moveup") {
                        thiss.x = -25
                        toggle = thiss.gif("wall_moveup")
                    } else if(thiss.y <= -imgsize) {
                        toggle();
                        state = 'down';
                    }
                } else if (state === 'down') {
                    if (thiss.y < windowHeight - imgsize) {
                        thiss.y += 10
                        marisa.css({ top: thiss.y });
                    } else {
                        return;
                    }
                }
    ...
    //     })
    //     this.observer.observe(document.getElementById("marisa"), { attributeFilter: ["style"] })
    // }
